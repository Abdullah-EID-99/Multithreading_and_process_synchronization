...
int main(int argc, char *argv[], char** envp){
    ...
    for(i=0;i<ph_count;i++){
        pthread_create(&ph_list[i].my_thread, NULL,ph_enter_DinoPhilo, i);
    }
    for(i=0;i<ph_count;i++){
        pthread_join(ph_list[i].my_thread, &ph_list[i].status);
    }
...
ph ph_creat(int id){
    ...
    p.amount_of_rice_this_ph_eat=0;
     if( pthread_mutex_init(&p.my_lock,NULL) != 0){
        printf("there is an error occurs in pthread_mutex_init(p.my_lock)\n");  
    }
    pthread_mutex_lock(&p.my_lock);
    time_t t;
...
void* ph_eat(int tb_id, int ph_id){
    ...
    ph_list[ph_id].amount_of_rice_this_ph_eat +=rice_that_can_be_eaten_per_time;
    tb_list[tb_id].riceAmount -=rice_that_can_be_eaten_per_time;
    tb_list[tb_id].eaten_rice_amount +=rice_that_can_be_eaten_per_time;
        ...
        if(!tb_list[i].is_opend){
            ...
            if(index == chairs_count-1){
                 ...
                 for(k=0;k<chairs_count;k++){
                    ...
                    pthread_mutex_unlock(&ph_list[ph_id].my_lock);
                    //printf("unlock ph %d\n",ph_id);
                }
...
void* ph_enter_DinoPhilo(int id){
    ...
    pthread_mutex_unlock(&enter_table_lock);
     ...
     if(!tb_list[table_id].is_opend){
        pthread_mutex_lock(&ph_list[id].my_lock);
    }
...
void destroy_threads(tb* t, int id){
    ...
    for(j=0;j<chairs_count;j++){
        ...
        if(ph_id !=id){
            pthread_detach(ph_list[ph_id].my_thread);
        }
